    ...
    public int longestConsecutive(int[] nums) {
        ...
        for(int i=0;i<nums.length;i++){
            ...
            if(numMap.containsKey(nums[i]-1)){
                ...
                Integer[] f = numMap.get(nums[i]-1);
                minFound = Integer.min(f[0],minFound);
                maxFound = Integer.max(f[1],maxFound);
            }
            if(numMap.containsKey(nums[i]+1)){
                Integer[] g = numMap.get(nums[i]+1);
                minFound = Integer.min(g[0],minFound);
                maxFound = Integer.max(g[1],maxFound);
            }
